...
public class TextLineNumber extends JPanel implements CaretListener, DocumentListener, PropertyChangeListener
{
		...
		while (rowStartOffset <= endOffset)
		{
			try
			{
			   ...
			   //calculate the position of the number
				...
				int x = getOffsetX(width, stringWidth) + insets.left;
				int y = getYOffset(rowStartOffset, fontMetrics);
	...
	private int getYOffset(int rowStartOffset, FontMetrics fontMetrics)
			throws BadLocationException
	{
		//  Get the bounding rectangle of the row

		Rectangle r = textComponent.modelToView( rowStartOffset );
		...
		else  // We need to check all the attributes for font changes
		{
			...
			for (int i = 0; i < line.getElementCount(); i++)
			{
				...
				if (fontMetrics1 == null)
				{
					...
					fontMetrics1 = textComponent.getFontMetrics( font );
	...
	/**
	 * Highlighted the line number on witch the caret is when the caret move
	 *
	 * @param e CaretEvent unused
	 */
	@Override
	...
	/**
	 * update the document line number
	 */
	private void documentChanged()
	{
		SwingUtilities.invokeLater(() -> {
			...
			try
			{
				...
				Rectangle rect = textComponent.modelToView(endPos);
			...
			catch (BadLocationException ignored) { } //do nothing on error
		});
	}
